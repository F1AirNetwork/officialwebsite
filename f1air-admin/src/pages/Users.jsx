...
import {
  ...
  Gift, XCircle, ShieldCheck, ShieldOff, ChevronDown,
} from "lucide-react";
...
const ActionBtn = ({ onClick, icon: Icon, label, danger, muted }) => (
  <button
    ...
    className={`flex items-center gap-1.5 px-2.5 py-1.5 text-xs rounded-lg font-medium transition-colors ${
      ...
      : muted ? "bg-zinc-800 hover:bg-zinc-700 text-zinc-400"
...
const SubCell = ({ user, onRemoveSingle, onRemoveAll }) => {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-1.5 text-xs font-semibold transition-colors text-green-400 hover:text-green-300"
      >
      ...
      {open && (
        <div className="absolute left-0 z-20 w-56 mt-1 overflow-hidden border shadow-2xl bg-zinc-900 border-zinc-700 rounded-xl">
          ...
          <div className="py-1 overflow-y-auto max-h-48">
            {items.map((item, i) => (
              <div key={item._id || i} className="flex items-center justify-between px-3 py-2 transition-colors hover:bg-zinc-800">
...
export default function Users() {
  ...
  const openGiftModal = async (user) => {
    setSelectedProduct(""); setGiftNote("");
    setGiftModal({ user });
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">{total} verified {total === 1 ? "user" : "users"}</p>
        </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={handleSearch} className="px-4 py-2.5 bg-brand hover:bg-brand-dark text-white text-sm font-semibold rounded-lg transition-colors">Search</button>
        {search && <button onClick={clearSearch} className="px-4 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 text-sm rounded-lg transition-colors">Clear</button>}
      </div>
                ...
                <tr key={u._id} className={`hover:bg-zinc-800/30 transition-colors ${u.isBanned ? "opacity-60" : ""}`}>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-medium text-white whitespace-nowrap">{u.firstName} {u.lastName}</p>
                  ...
                  <td className="px-5 py-4">
                    <div className="flex flex-col gap-1.5">
                      ...
                      <button
                        ...
                        className="self-start inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 transition-colors"
                      >
                  ...
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      <ActionBtn
                        ...
                        danger={!u.isBanned} muted={u.isBanned}
                      />
                    </div>
                  </td>
                </tr>
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                { label: "Total Orders",     value: detailUser.recentOrders?.length ?? 0 },
              ].map(({ label, value }) => (
            ...
            <div className="p-4 rounded-lg bg-zinc-800">
              <p className="mb-3 text-xs font-semibold tracking-widest uppercase text-zinc-400">Active Purchases</p>
              {(() => {
                const paid = detailUser.recentOrders?.filter((o) => o.status === "paid") || [];
                if (detailUser.subscription?.status === "active" && paid.length === 0) {
                  return (
                    <div className="flex items-center justify-between">
                      <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={() => { handleRemoveSub(detailUser); setDetailUser(null); }}
                          disabled={actioning}
                ...
                if (paid.length === 0) return <p className="text-sm text-zinc-500">No active purchases</p>;
                return (
                  <div className="space-y-2">
                    {paid.map((o) => (
                      <div key={o._id} className="flex items-center justify-between px-3 py-2 rounded-lg bg-zinc-900">
                        <div>
                          <p className="text-xs font-medium text-white">{o.productName}</p>
                          <p className="text-[10px] font-mono text-zinc-600">#{o._id.slice(-6).toUpperCase()}</p>
                        </div>
                        <div className="flex items-center gap-2">
                          <Badge color="green" dot>paid</Badge>
                          <span className="text-xs text-zinc-400">{o.amount === 0 ? "Free" : `$${o.amount}`}</span>
                        </div>
                      </div>
                    ))}
                  </div>
            ...
            {detailUser.recentOrders?.length > 0 && (
              <div>
                <p className="mb-2 text-xs font-semibold tracking-widest uppercase text-zinc-400">All Orders</p>
                <div className="space-y-2 overflow-y-auto max-h-48">
                  {detailUser.recentOrders.map((o) => (
            ...
            <div className="flex flex-wrap gap-2 pt-2 border-t border-zinc-800">
              <button onClick={() => { handleRoleToggle(detailUser); setDetailUser(null); }} disabled={actioning}
                ...
                <Shield size={14} /> {detailUser.role === "admin" ? "Remove Admin" : "Make Admin"}
              </button>
              <button onClick={() => { setDetailUser(null); openGiftModal(detailUser); }}
              ...
              <button onClick={() => { setDetailUser(null); openBanModal(detailUser); }} disabled={actioning}
                className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg transition-colors ${
                  detailUser.isBanned ? "bg-green-500/10 hover:bg-green-500/20 text-green-400" : "bg-red-500/10 hover:bg-red-500/20 text-red-400"
                }`}>
                {detailUser.isBanned ? <><ShieldCheck size={14} /> Unban</> : <><Ban size={14} /> Ban</>}
              </button>
              <button onClick={() => setDetailUser(null)} className="px-4 py-2 ml-auto text-sm transition-colors rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-300">Close</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      {/* ══════════ CURRENCY MODAL ══════════ */}
      <Modal open={!!currencyModal} onClose={() => setCurrencyModal(null)}
        title={`Set Currency — ${currencyModal?.user?.firstName || ""} ${currencyModal?.user?.lastName || ""}`} width="max-w-sm">
        {currencyModal && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 px-4 py-3 text-sm rounded-lg bg-zinc-800/60">
              ...
              {currencyModal.user.country && <span className="ml-auto text-xs text-zinc-500">Country: {currencyModal.user.country}</span>}
            </div>
            <div>
              <label className="block mb-2 text-xs font-semibold tracking-widest uppercase text-zinc-400">Country Code</label>
              <input value={currencyForm.country} maxLength={2} placeholder="e.g. IN, US, DE"
            ...
            <div>
              <label className="block mb-2 text-xs font-semibold tracking-widest uppercase text-zinc-400">Currency Override</label>
              <div className="grid grid-cols-3 gap-2">
                {[{ code:"USD",symbol:"$",label:"US Dollar" },{ code:"INR",symbol:"₹",label:"Indian Rupee" },{ code:"EUR",symbol:"€",label:"Euro" }].map(({ code, symbol, label }) => (
                  <button key={code} type="button" onClick={() => setCurrencyForm((f) => ({ ...f, currency: code }))}
                    className={`flex flex-col items-center gap-0.5 py-2.5 rounded-lg border transition-colors ${
                      currencyForm.currency === code ? "border-brand bg-brand/10 text-brand" : "border-zinc-700 bg-zinc-800 text-zinc-400 hover:border-zinc-500"
                    }`}>
            ...
            <div className="flex gap-3 pt-2 border-t border-zinc-800">
              <button onClick={handleCurrencyUpdate} disabled={actioning}
              ...
              <button onClick={() => setCurrencyModal(null)} className="px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-sm font-semibold transition-colors">Cancel</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      {/* ══════════ BAN MODAL ══════════ */}
      <Modal open={!!banModal} onClose={() => setBanModal(null)}
        title={banModal?.user?.isBanned ? `Unban ${banModal?.user?.firstName}?` : `Ban ${banModal?.user?.firstName}?`} width="max-w-md">
        {banModal && (
          <div className="space-y-4">
            {banModal.user.isBanned ? (
              <div className="px-4 py-3 border rounded-lg bg-green-500/10 border-green-500/20">
                <p className="text-sm text-green-400">This will restore {banModal.user.firstName}'s access immediately.</p>
                {banModal.user.banReason && <p className="mt-1 text-xs text-green-300/70">Previous reason: {banModal.user.banReason}</p>}
              </div>
            ) : (
              <>
                <div className="px-4 py-3 border rounded-lg bg-red-500/10 border-red-500/20">
                  <p className="text-sm text-red-400">Banning <strong>{banModal.user.firstName} {banModal.user.lastName}</strong> will immediately block all login attempts.</p>
                </div>
                <div>
                  <label className="block mb-2 text-xs font-semibold tracking-widest uppercase text-zinc-400">Reason *</label>
            ...
            <div className="flex gap-3 pt-2 border-t border-zinc-800">
              <button onClick={handleBan} disabled={actioning}
                className={`flex items-center gap-2 font-semibold px-5 py-2.5 rounded-lg text-sm transition-all disabled:opacity-50 ${
                  banModal.user.isBanned ? "bg-green-600 hover:bg-green-700 text-white" : "bg-red-600 hover:bg-red-700 text-white"
                }`}>
                {actioning ? <Spinner size={14} /> : null}
                {banModal.user.isBanned ? "Yes, Unban User" : "Yes, Ban User"}
              </button>
              <button onClick={() => setBanModal(null)} className="px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-sm font-semibold transition-colors">Cancel</button>
            </div>
          </div>
        )}
      </Modal>
      ...
      {/* ══════════ GIFT MODAL ══════════ */}
      <Modal open={!!giftModal} onClose={() => setGiftModal(null)}
        title={`Gift a Product to ${giftModal?.user?.firstName}`} width="max-w-md">
        {giftModal && (
          <div className="space-y-4">
            <div className="flex items-center gap-3 px-4 py-3 border rounded-lg bg-brand/10 border-brand/20">
              ...
              <p className="text-sm text-zinc-300">
                The gifted product will be applied immediately to <strong className="text-white">{giftModal.user.firstName} {giftModal.user.lastName}</strong>'s account.
              </p>
            </div>
            <div>
              ...
              {products.length === 0 ? (
                ...
                <select value={selectedProduct} onChange={(e) => setSelectedProduct(e.target.value)}
                  ...
                  {products.map((p) => (
                    <option key={p._id} value={p._id}>{p.name}  ·  ${p.price}/{p.billingCycle}  [{p.type}]</option>
                  ))}
                </select>
              )}
            </div>
            {selectedProduct && (() => {
              ...
              return <div className="px-4 py-3 text-xs rounded-lg bg-zinc-800 text-zinc-400">{hints[p.type] || ""}</div>;
            })()}
            <div>
              <label className="block mb-2 text-xs font-semibold tracking-widest uppercase text-zinc-400">Internal Note <span className="font-normal normal-case text-zinc-600">(optional)</span></label>
              <input value={giftNote} onChange={(e) => setGiftNote(e.target.value)}
                placeholder="e.g. Compensation for downtime..."
                className={inputCls} />
            </div>
            <div className="flex gap-3 pt-2 border-t border-zinc-800">
              <button onClick={handleGift} disabled={actioning || !selectedProduct}
                className="flex items-center gap-2 bg-brand hover:bg-brand-dark disabled:opacity-50 text-white font-semibold px-5 py-2.5 rounded-lg text-sm transition-all">
                {actioning ? <Spinner size={14} /> : <Gift size={14} />}
                {actioning ? "Gifting..." : "Gift Now"}
              </button>
              <button onClick={() => setGiftModal(null)} className="px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded-lg text-sm font-semibold transition-colors">Cancel</button>
            </div>
          </div>
        )}
      </Modal>
